// /d/Praxis/setter.c
// Character creation room - race and OCC selection for RiftsMUD
...
void create() {
    ...
    set_items(
        (["room" : "The void from which all characters are born.",
          "list" : "A list of playable races in RiftsMUD."]) );
}
...
void do_rolls() {
    ...
    if((int)RIFTS_D->is_rifts_race(race)) {
        ...
        /* Rifts damage pool stats */
        if((int)RIFTS_D->is_mdc_race(race)) {
            /* MDC beings: MDC is primary combat pool; small SDC buffer for SDC weapon hits */
        ...
        // Palladium HP (PE + die roll) stored as rifts_hp/max_rifts_hp.
        // The D&D shadow key "constitution" = PE continues to drive Nightmare's
        // combat HP pool via living.c's 50+10*CON formula.
        hp = (int)RIFTS_D->init_hp(race, borg["PE"]);
        this_player()->set_stats("rifts_hp", hp);
        this_player()->set_stats("max_rifts_hp", hp);

        // Race ability flags stored in env (persistent across logins)
        rflags = (string *)RIFTS_D->query_race_flags(race);
