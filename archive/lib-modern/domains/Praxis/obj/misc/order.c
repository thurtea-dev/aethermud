...
private nosave string create_file();

void create() {
    Object::create();
    set_name("work order");
    set_id( ({ "order", "work order" }) );
    set_short("a work order");
    set_long("An order for workers to begin modifying your estate "
...
protected void input_exit(string str) {
    ...
    seteuid(UID_ESTATES);
    write_file(file, "add_exit: $"+__NewRoom+";$"+str+"\n");
