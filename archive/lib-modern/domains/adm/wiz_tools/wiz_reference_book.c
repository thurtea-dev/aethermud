/* /domains/adm/wiz_tools/wiz_reference_book.c
   ...
   Set property "book_role" to: admin, coding, rp, or domain.
   Type: read book
...
private string book_content(string role) {
    ...
    if(role == "coding") {
        return
        "=== RiftsMUD Coding Wizard (quick ref) ===\n\n"
    ...
    if(role == "rp") {
        return
        "=== RiftsMUD Roleplay Wizard (quick ref) ===\n\n"
    ...
    if(role == "domain") {
        return
        "=== RiftsMUD Domain Wizard (quick ref) ===\n\n"
        "position: domain_wiz   wiz_role: domain\n"
        "Tool required: staff of dominion (dominate)\n\n"
        "DOMINION MENU\n"
