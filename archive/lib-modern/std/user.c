...
private nosave register_channels();
...
void write_messages() {
    ...
    if(getenv("TERM") == "unknown" && (string)getenv("colorize_set") != "1")
        message("system",
            "\nTerminal type not detected. Type 'colorize on' to enable ANSI color.",
            this_object());
    ...
    if(entering_wiz) {
        ...
        if(member_array(prev_pos, MORTAL_POSITIONS) != -1) {
            ...
            tell_object(this_object(),
                "You have been promoted.  Welcome to the staff of RiftsMUD.\n"
