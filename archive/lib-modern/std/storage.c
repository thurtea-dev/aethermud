...
private nosave int __Closed, __Locked;
    ...
    if(tmp = (int)ob->move(this_object())) {
        if(query_closed()) {
            if(query_transparent())
              message("my_action", "The lid is closed.", this_player());
            else
...
int query_contents_visible() {
    if(!query_can_close() || !query_closed()) return 1;
...
varargs string query_long(string str) {
    ...
    if(query_can_close() && query_closed()) {
        if(query_contents_visible()) {
