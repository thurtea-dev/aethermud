...
private nosave string allowed_to_follow;
private nosave object *followers;
private nosave object following;
...
void move_followers(object prev) {
	...
	if(environment(followers[i]) != prev) {
	    message("my_action","You have lost "+
                (string)followers[i]->query_display_name(this_object())+".",
                this_object());
	    message("other_action",
                (string)this_object()->query_display_name(followers[i])+
                " has ditched you.",followers[i]);
