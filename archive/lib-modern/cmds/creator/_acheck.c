...
//      a tool for the balancing and checking of armor
...
check_armor(object ob)
{
    ...
    is_armor = (int)ob->is_armour();
    if (!intp(is_armor)) {
	write("Warning: is_armor() did not return an int.\n");
	error = 1;
    } else
	if (!is_armor) {
	    write("Warning: is_armor() did not return true.\n");
	    error = 1;
	}
	...
	if (member_array(type, ARMOUR_TYPES) == -1) {
	    write("Warning: query_type() returned unknown armor " +
		"type: " + type + ".\n");
	    error = 1;
	}
...
int
cmd_acheck(string armor)
{
    object ob;

    if (!stringp(armor)) {
	notify_fail("usage: " + query_verb() + " <armor-id>\n");
	return 0;
    }

    ob = present(armor, this_player());
    if (!objectp(ob)) {
	notify_fail(query_verb() + ": \"" + armor + "\" not present.\n");
	return 0;
    }

    path = base_name(ob);

    write("Armor Check for: " + path + "\n");
