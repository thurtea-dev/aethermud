/* /daemon/rifts.c */
/* RiftsMUD  -  Palladium Rifts race and attribute daemon */
...
int is_mdc_weapon(object weapon) {
    ...
    dmg = lower_case((string)weapon->query_property("damage_string"));
    if(dmg && sizeof(dmg) && strsrch(dmg, " md") != -1) return 1;
    return 0;
}
...
private int leaves_blood_pool(object victim) {
    ...
    race = lower_case((string)victim->query_race());
...
int query_rest_interval(object pl) {
    ...
    if(!occ || !sizeof(occ))
        occ = lower_case((string)pl->query_property("rifts_occ"));
