...
private nosave int __SocketHTTP;
private nosave mapping __Sockets, __Activity;
...
protected void setup() {
    /* HTTP daemon disabled - RiftsMUD uses only ports 1122 and 1199. */
    return;
}
...
private nosave void http_error(int fd, mapping err) {
    string str;
...
private nosave void add_activity(int fd, string act) {
...
private nosave void get_file(int fd, string file) {
    string *parts, *tmp;
    mixed str;
    string id, args;
    int x;
