...
protected int name_matches(string topic, string name) {
    string norm;
    string underscored;
...
protected int is_alignment_topic(string topic) {
    string norm;
...
protected int is_skills_topic(string topic) {
    string norm;
...
protected int is_combat_topic(string topic) {
    string norm;
    ...
    return member_array(norm, ({
        ...
        "death", "reload", "unload", "wimpy", "whimpy", "hide", "search",
        "position", "rest", "wake"
    })) != -1;
}

protected int is_comm_topic(string topic) {
    string norm;
...
protected int is_systems_topic(string topic) {
    string norm;
    ...
    return member_array(norm, ({
        ...
        "color", "colorize", "colour", "days", "months", "moons", "timezones",
        "syntax", "races", "occs", "classes", "password", "lock", "look",
        "sirname"
    })) != -1;
}

protected int is_race_topic(string topic, string first_line) {
    string *races;
    string lower_first;
    int i;
...
protected int is_occ_topic(string topic, string first_line) {
    string *occs;
    string lower_first;
    int i;
...
protected void flat_help_display() {
    ...
    for(i = 0; i < sizeof(topics); i++) {
        ...
        switch(sec) {
        case HELP_SEC_RACES:    sec_races    += ({ topics[i] }); break;
        case HELP_SEC_OCCS:     sec_occs     += ({ topics[i] }); break;
