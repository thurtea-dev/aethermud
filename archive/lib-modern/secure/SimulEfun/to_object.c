...
protected object resolve_env(object p, int outside)
{
   if (outside) {
...
object to_object(mixed target)
{
   ...
   if (objectp(target)) { /* already an object */
      ...
      if (str == "here") {
         return resolve_env(environment(this_player()), outside);
      }
      if (str == "me") {
         return resolve_env(this_player(), outside);
      }
      if (!env) {
         env = this_player();
      }
      ...
      case '/' : /* filename */
         ...
         return resolve_env(load_object(str), outside);
      case '#' : /* nth object */
         list = all_inventory(env);
         sscanf(str, "%d", which);
         return resolve_env(list[which], outside);
      case '$' : /* monster */
         p = find_living(str);
