...
private nosave mapping file_mapping(string *files);
private nosave string display_ls(mixed targ, int aflag, int lflag, int tflag,
  int nflag, int bflag, int sflag);
private nosave string long_list(string dir, mixed *files);
private nosave string short_list(string dir, mixed *files, int n, int s);
...
int cmd_ls(string str) {
    ...
    dirs = filter_array(files, "is_dir", this_object());
...
private nosave string long_list(string dir, mixed *files) {
    ...
    if(member_array(dir[0..strlen(dir)-2],
      (string *)previous_object()->query_search_path()) != -1) acc += "x";
    else acc += "-";
...
protected string map_files(mixed *file, int *flags) {
    ...
    if(!flags[1]) {
        if(flags[0] && file[0] && tmp2 && stringp(tmp2)  && sizeof(tmp2) > 2 &&
find_object(tmp2) && file[1] != -2) tmp = "*";
        else tmp = " ";
    }
    else tmp = "";
    if(!flags[2]) {
