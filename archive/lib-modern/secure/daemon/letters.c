...
private nosave string __LetterId;
private nosave mapping __LettersDir, __PostalDir; 
private nosave mixed *__LetterPtr, *__PostalPtr;
...
void create() { 
    ...
    if( tmp && sizeof(tmp)) __PostalPtr = ({ tmp[0], 0 }); 
    while(i--) { 
...
private nosave int valid_access() {
    switch(base_name(previous_object(0))) {
        case OB_POSTAL: case LETTERS_D: case FOLDERS_D:
...
private nosave void save_letter() { 
    string file;
    int x;
...
private nosave int restore_letter(string id) { 
    string dir; 
    int x; 
...
string query_letter(string id) { 
    ...
    if((base = base_name(previous_object(0))) != OB_POSTAL &&
...
protected void manage_letters() {
    ...
    if(!(x = sizeof(dir_keys = keys(__LettersDir)))) return;
    if(__LetterPtr[1] >= sizeof(__LettersDir[__LetterPtr[0]])) {
        ki = member_array(__LetterPtr[0], dir_keys);
        if(++ki >= x) __LetterPtr = ({ dir_keys[0], 0 });
        else __LetterPtr = ({ dir_keys[ki], 0 });
      }
    // Guard: if current key no longer valid, reschedule
    if(!__LettersDir[__LetterPtr[0]] ||
       __LetterPtr[1] >= sizeof(__LettersDir[__LetterPtr[0]])) {
