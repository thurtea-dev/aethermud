...
private nosave int __SocketMCP;
private nosave mapping __Sockets;
private nosave mapping __ValidHosts;
...
protected void setup() {
    /* MCP/Intermare daemon disabled - RiftsMUD uses only ports 1122 and 1199. */
    return;
}
...
private nosave void retrieve_file(int fd, string file) {
    string str;
    int i;
...
private nosave void store_file(int fd, string file, string *lines) {
    int i, j, eof;
...
private nosave void do_send(string file,string dest) {
    int fd;
    if (strlen(file) < 5 || file[0..4] != "/ftp/") {
