...
private nosave string __Folder, __Owner;
...
private nosave void load_folder(string who, string folder) {
    string file;
...
private nosave void save_folder() {
    string file;
...
mapping *query_box_info(string who, string folder) {
    if(base_name(previous_object(0)) != OB_POSTAL) return ({});
...
void add_post(string who, string folder, mapping borg) {
    ...
    if(previous_object(0) != this_object() && tmp != OB_POSTAL &&
