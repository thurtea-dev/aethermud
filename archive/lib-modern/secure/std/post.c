...
private nosave int __Screen, __Lines, __NumLetters;   
private nosave int __Begin,__Current,__CommandLine,__FromMenu,__IncomingFlag;
private nosave string __Folder, __Owner, __CurrentMenu;    
private nosave mapping __Options, __TmpPost, __ChangedOptions;    
private nosave int *__Delete; 
private nosave string *__FwdRply;    
private nosave mapping *__BoxInfo, *__PostalOptions;    
    
void create() {    
    Object::create();    
    set_name("post box");    
    set_id( ({ "post box", "box", POSTAL_ID }) );    
    set_adjectives( ({ "post", "a", "imaginary" }) );    
    set_short("a post box");    
    set_long("A post box.");    
...
private nosave int set_current(mixed val) {
    int i;
...
varargs protected void options(string str) {    
    ...
    for(i=0; i<maxi; i++) {    
        ...
        message("mail", sprintf("%s) %-45s %s", (i+1>9) ? (i+1)+"" :" "+(i+1),
...
private nosave void help(string arg, string ind) {    
    ...
    message("mail", "\n%^INITTERM%^Imaginary Intermud Postal Service "    
      "(IIPS) 3.1     Descartes of Borg 1993, 1994", this_player());    
    message("mail",sprintf("\n%%^CYAN%%^%s",center("IIPS 3.1 Help Menu",   
      __Screen)), this_player());    
    help_menu(ind);    
}    
...
private nosave void alias_menu() {    
    ...
    message("mail", center("r)emove an alias", __Screen),this_player());    
    primary_prompt();    
    input_to("alias_cmd");    
  }    
...
private nosave void option_menu() {    
    ...
    message("mail", center("a)lias menu, h)elp, i)ndex menu, "    
      "q)uit, Q)uit without saving,", __Screen), this_player());    
    message("mail", center("s)ave option changes", __Screen), this_player()); 
    primary_prompt();    
    input_to("option_cmd");    
}    
    ...
    switch(ind) {  
        case "index":  
            ...
            message("help", center("x) help from another menu, y) detailed "  
              "IIPS user manual", __Screen), this_player());  
            break;  
        case "alias":  
...
protected void next_folder(string str, string folder) {
    ...
    if(str == "y") save_box();
        ...
        message("prompt", "Invalid command.  Enter 'y' or 'n': ",this_player());
        ...
        if(cmd == "u" && !__Delete[i]) {    
          ...
          !__Options["unread delete"]) {    
            message("mail", sprintf("\n%%^RED%%^Letter %d is still unread!",   
              i+1), this_player());  
            message("prompt","Delete it anyways (default n): ",this_player());   
...
protected void confirm_quit(string str, string cmd) {   
    ...
    message("prompt", "Answer 'y' or 'n': ", this_player());   
    input_to("confirm_quit", cmd);   
  }   
...
private nosave void save_letter(string cmd, string args) {   
    ...
    if(!folder) {   
        if(cmd == "S") message("prompt","Save to which file? ",this_player());   
    ...
    if(cmd == "s") {   
        ...
        message("mail", sprintf("Letter saved to %s.", folder),    
          this_player());   
      set_current(letter);
        __Delete[__Current] = 1;   
      }   
...
private nosave void alias_members(string cmd, string args) {   
    ...
    if(cmd == "d") {   
        if(!members)    
          message("mail", "\n%^GREEN%^All members deleted.", this_player());   
        else message("mail", "\n%^GREEN%^Members deleted from alias.",   
          this_player());   
      }   
    else message("mail", "\n%^GREEN%^Members entered into alias.",
      this_player());   
    if(__CommandLine) this_object()->remove();
    else {
        secondary_prompt();   
        input_to("alias_cmd");
    }   
...
private nosave void alias_creation(string cmd, string args) {  
    ...
    if(cmd == "m" && sizeof(members=explode(args=lower_case(args)," ")) == 1) {  
        message("prompt", "Enter members for the alias: ", this_player());  
...
private nosave void change_option(int x) {  
    ...
    message("prompt", "Enter in a proper value: ", this_player());  
...
private nosave void reply(string str) {
    ...
    set_current(x);
    __TmpPost = ([ "date":time(), "from": (string)this_player()->query_name()]);
    if((__TmpPost["subject"] = __BoxInfo[x]["subject"])[0..2] != "Re:") 
      __TmpPost["subject"] = sprintf("Re: %s", __TmpPost["subject"]);
    message("prompt", "Include original text (default 'n'): ", this_player());
...
protected void get_reply_confirm(string str) {
    ...
    message("prompt", "\nEnter choice (default 's'): ", this_player());
...
protected void get_reply_list(string str) {
    ...
    switch(str) {
        ...
        case "t":
            ...
            break;
        default:
            message("prompt","Invalid choice.  Choose again: ", this_player());
            input_to("get_reply_list");
            return;
          }
...
private nosave string query_reply_text() {
    string tmp;
...
private nosave void send_letter(string *args) { 
    ...
    __FwdRply = 0;
    __TmpPost = ([ "from": (string)this_player()->query_name(),"date":time()]);
...
protected void get_cc(string str) { 
    ...
    if(file_exists(tmp = DIR_TMP+"/"+(string)this_player()->query_name()))
      rm(tmp);
    if(!__FwdRply || __FwdRply[0] == "f") { 
        message("mail",sprintf("\n%%^INITTERM%%^%s", header(__TmpPost)), 
          this_player()); 
        this_player()->edit(tmp, (:complete_send:), (:abort:)); 
        return; 
    } 
...
void complete_send() { 
    ...
    if(!(str = read_file((string)this_player()->query_edit_filename())))
      str = "No message."; 
    if(__FwdRply && __FwdRply[0] == "f")
...
private nosave string query_signature() { 
    ...
    tmp = absolute_path(user_path((string)this_player()->query_name()), 
      __Options["sig file"]); 
...
protected void handle_send_choice(string str) {
    ...
    switch(str) {
        ...
        case "f":
            ...
            break;
        case "e":
            if(file_exists(tmp = sprintf("%s/%s.iips", DIR_TMP, 
              (string)this_player()->query_name()))) rm(tmp);
            write_file(tmp, __TmpPost["message"]);
            map_delete(__TmpPost, "message");
            this_player()->edit(tmp, (:complete_send:), (:abort:));
            break;
        default:
            message("prompt", "Invalid command.  Command: ", this_player());
            input_to("handle_send_choice");
            break;
    }
}
...
private nosave void notify_send(string *failures) { 
    ...
    if(!sizeof(failures)) 
      message("mail", "\n\t%^GREEN%^Mail successfully sent!", this_player()); 
        ...
        if(creatorp(this_player())) { 
            write_file(tmp=user_path((string)this_player()->query_name())+"/dead.letter", 
              __TmpPost["message"]);
          } 
        else write_file(tmp = sprintf("%s/%s.letter", DIR_TMP, 
          (string)this_player()->query_name()), __TmpPost["message"]); 
