...
private nosave int __CrackCount, __CopyExists; 
private nosave string __Name, __CapName, __Client; 
private nosave object __Player; 
 
protected void logon();
protected void get_name(string str);
string query_name();
protected void get_password(string str);
private nosave int locked_access();
private nosave int check_password(string str);
private nosave int valid_site(string ip);
private nosave int boot_copy();
protected void disconnect_copy(string str);
private nosave void exec_user();
...
private nosave void internal_remove();
void remove();
...
protected void continue_login() {
   ...
   if(!__Player){
    ...
    if(__Player && (int)__Player->query_property("permanently_dead")) {
        ...
        if(!pd_name || pd_name == "")   pd_name   = capitalize(__Name);
