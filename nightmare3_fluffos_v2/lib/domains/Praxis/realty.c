...
void create() {
    ...
    set_long(
      ...
      "up in all kinds of dreary colors, with a desk at the far end of the "
      "office being the spot where all business is done."
    );
    set_items( ([ "colors": "Mostly puce.", "desk": (: "at_desk" :),
...
void reset() {
   ...
   ob->set_properties( (["no paralyze":1, "no steal":1]));
    ...
    arm->set_long("A beautifully crafted, fake-silver plate armor once "
