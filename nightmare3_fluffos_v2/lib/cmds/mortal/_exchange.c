...
int cmd_exchange(string str) {
    ...
    /* sscanf counts %d as matched even when the trailing literal fails,
       so "%d credits to black" would also match "50 black to credits"
       and convert the wrong way; capture the tail with %s and compare. */
    if(sscanf(rest, "%d %s", amount, tail) == 2) {
        if(tail == "credits to black")
            return convert_credits_to_black(target, tp, amount, service);
        if(tail == "black to credits")
