...
int cmd_whisper(string str) {
  ...
  if(ob->is_player() && !interactive(ob)) {
    notify_fail((string)ob->query_display_name(this_player()) +
                " is link-dead and cannot hear you.\n");
    return 0;
  }
  tell_object(ob, "%^BOLD%^%^CYAN%^"+
      (string)act_ob->query_display_name(ob)+
                              " whispers to you: %^RESET%^"+msg);
  write("%^BOLD%^%^CYAN%^You whisper to "+
      (string)ob->query_display_name(this_player())+": %^RESET%^"+
	msg);
  env = environment(act_ob);
  inv = all_inventory(env);
  for(i = 0; i < sizeof(inv); i++) {
      if(!inv[i] || inv[i] == act_ob || inv[i] == ob) continue;
      att_name = (string)act_ob->query_display_name(inv[i]);
      ob_name = (string)ob->query_display_name(inv[i]);
      tell_object(inv[i], "%^CYAN%^" + att_name +
          " whispers something to " + ob_name + ".%^RESET%^\n");
  }
  return 1;
}
