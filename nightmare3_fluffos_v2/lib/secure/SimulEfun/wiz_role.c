 ...
 * AetherMUD wizard role helpers. position is canonical; wiz_role is synced tag.
 */
...
int apprentice_wizp(object ob) {
    if(!ob) ob = previous_object();
    if(!ob || !creatorp(ob)) return 0;
    return lower_case((string)ob->query_position()) == "apprentice";
}
