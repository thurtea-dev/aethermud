...
### a. APM (Actions Per Melee) - RESOLVED (verified 2026-07-28)
...
A per-character action timer exists in `daemon/rifts_combat.c`: `ROUND_DURATION` (6s) drives `combat_round()`/`start_next_round()`, resolving a full APM block per side each round. This note predates that implementation.
...
Not reverified 2026-07-28 - out of scope for that session's pass, status unconfirmed either way.

### c. PPE Regeneration - RESOLVED (verified 2026-07-28)
Tabletop rate: 2 PPE/hour rest, 10 PPE/hour sleep, doubled near ley line.
`daemon/rifts_combat.c` `do_rifts_regen()` is live, called from `std/living.c` `heart_beat()`. This note predates that implementation.

### d. ISP Regeneration - RESOLVED (verified 2026-07-28)
Mind Melter source note in the race section states 2 ISP/hour active, 12 ISP/hour sleep.
Covered by the same `do_rifts_regen()` heartbeat as PPE (c.), which checks both pools. This note predates that implementation.

### e. MDC Damage Scale - RESOLVED (verified 2026-07-28)
...
`rifts_combat.c` `apply_rifts_damage()` explicitly checks `has_mdc_weapon`/`is_mdc_def` and bounces SDC attacks off MDC defenders with no damage (see docs/ASSESSMENT.md section on MDC/SDC). This note predates that implementation.

### f. Coalition Alignment Restrictions - PARTIALLY RESOLVED (verified 2026-07-28)
Alignment system is fully implemented (rifts_alignment env var, 7-tier numeric system, kept in sync with the numeric NM3 scale by `rifts_combat.c` `npc_died()`).
`domains/Praxis/setter.c` blocks Miscreant/Aberrant/Diabolic characters from picking any of the six CS military OCCs (cs grunt, cs dead boy, cs ranger, cs military specialist, cs technical officer, cs psi-stalker) at chargen; verified the alignment-before-OCC step order makes this live, not dead code. Scope limit, not a bug: no other OCC/alignment pairings (e.g. Cyber-Knight) are gated. Extending beyond CS is a content decision, not a fix.
