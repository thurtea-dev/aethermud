...
void apply_telepathy_send(object target, string message) {
    ...
    /* Deliberately does NOT seed the "reply" property. That property drives
       secure/cmds/mortal/_reply.c, which rides the staff-only tell channel.
       Seeding it here handed the recipient a free cross-room reply to
       another player that skipped telepathy's own ISP cost, knows_player()
       introduction gate, and mind_blocked check. Telepathy is answered with
       telepathy. See CLAUDE.md rule 20. */
