...
string query_active(object who) {
    ...
    known = parse_langs(lang_env);
    ks = keys(known);
    /* Highest proficiency wins, not ks[0]: same non-deterministic
       mapping-order pitfall grant_native_languages() had. */
    best = 0;
    best_pct = -1;
    for(i = 0; i < sizeof(ks); i++) {
        if(known[ks[i]] > best_pct) {
            best_pct = known[ks[i]];
            best = ks[i];
        }
    }
...
void grant_native_languages(object who) {
    ...
    active = (string)who->getenv("active_language");
    if(!active || active == "") {
        /* Pick by highest granted proficiency, not keys(native)[0]:
           mapping key order is not guaranteed to match the literal's
           declared order, so races with more than one native language
           (dragons, elves, atlanteans, ...) could get any of them as
           the "default" rather than the intended primary tongue. */
        best = 0;
        best_pct = -1;
        for(i = 0; i < sizeof(langs); i++) {
            if(native[langs[i]] > best_pct) {
                best_pct = native[langs[i]];
                best = langs[i];
            }
        }
        who->setenv("active_language", best ? best : "American");
    }
}
