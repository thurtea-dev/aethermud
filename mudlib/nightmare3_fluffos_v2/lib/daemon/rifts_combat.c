...
/* AetherMUD  -  Palladium combat statistics and damage daemon */
...
int apply_rifts_damage(object attacker, object defender, int raw_damage) {
...

    {
        ...
        /* Wild fire vulnerability (2026-07-28): secondary/wild vampires take
           double damage from fire. Bound to the generic "fire" element tag
           rather than a narrower "mundane fire only" one -- broad-and-simple
           per decision, so this also doubles magic fire (e.g. fireball) and
           tech plasma weapon damage, which strict Palladium canon excludes.
           Documented as a known limitation in doc/help/user/secondaryvampire
           and wildvampire; revisit only if it causes real balance problems. */
        if(elem == "fire") {
            dflags = (string *)RIFTS_D->query_race_flags((string)defender->query_race());
            if(dflags && member_array("wild_fire_vulnerability", dflags) != -1)
                raw_damage *= 2;
        }
    }
...
/* Sunlight vulnerability (2026-07-26). Implements the rule that was
   already documented but never coded: daemon/rifts.c query_race_flags()
   tags secondary/wild vampires with "sunlight_vulnerability", and
   query_race_data()'s penalties array plus doc/help/user/secondaryvampire
   and wildvampire all promise "Sunlight: 2d6 MDC per melee round of
   exposure". Nothing read that flag until now.

   Driven from std/living.c heart_beat() on a ROUND_DURATION gate, the same
   shape as the do_rifts_regen() hook next door, so NPC vampires
   (domains/Praxis/monsters/wild_vampire.c) burn on the same terms players
   do. Damage goes through apply_direct_damage() per the whole-body model:
   barriers, then worn armor pools, then MDC -- so Armor of Ithan or a
   sealed environmental suit genuinely buys a vampire time outdoors, which
   is the intended counterplay.

   Guards are ordered cheapest-first on purpose: this runs for every living
   thing on every gated heartbeat, and the race-flag switch in rifts.c is
   by far the most expensive step, so it is reached only by something
   already standing outdoors in daylight. Time of day comes from
   EVENTS_D->query_time_of_day() (night/dawn/day/twilight); there is no
   weather system in this lib, so exposure is purely time-of-day driven.
   Twilight and night are safe. */
int do_sunlight_exposure(object who) {
    object env;
    string tod;
    string race;
    string *flags;
    ...
    env = environment(who);
    if(!env) return 0;
    if((int)env->query_property("indoors")) return 0;
    if((int)who->query_ghost()) return 0;
    tod = (string)EVENTS_D->query_time_of_day();
    if(tod != "day" && tod != "dawn") return 0;
    race = (string)who->query_race();
    if(!race || !strlen(race)) return 0;
    flags = (string *)RIFTS_D->query_race_flags(race);
    if(!flags || member_array("sunlight_vulnerability", flags) == -1) return 0;

    dmg = random(6) + random(6) + 2;   /* 2d6 */
    tell_object(who,
        "Direct sunlight sears your undead flesh!\n");
    if(who->query_cap_name())
        tell_room(env, (string)who->query_cap_name() +
            "'s flesh blisters and smokes in the sunlight.\n", ({ who }));
    apply_direct_damage(who, dmg);
    return dmg;
}
