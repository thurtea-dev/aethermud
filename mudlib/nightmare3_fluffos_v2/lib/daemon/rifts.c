...
/* Player-facing display formatting for spell / skill / psionic names
   (2026-07-26). Capitalizes every word: "armor of ithan" -> "Armor Of
   Ithan", "sense evil" -> "Sense Evil".

   DISPLAY ONLY. Canonical keys, "effect" strings, help-file names and every
   lookup path are untouched. Note the binding command-naming rule in
   CLAUDE.md: a multi-word ability's canonical key is a single lowercase word
   with no separator, so some keys are stored collapsed and NO capitalizer
   can recover their word boundaries -- "swimasafish" cannot be split by
   rule. Those get an explicit entry below; everything else falls through to
   the generic word-capitalizer.

   Because this makes spaced forms visible to players who will type back what
   they were shown, rifts_spells_d.c normalize_spell_name() carries matching
   spaced aliases ("swim as a fish" -> "swimasafish") so the displayed form
   also casts. Add to both places together or discoverability breaks. */
string display_ability_name(string name) {
    string *words;
    string out;
    int i;
    ...
    case "swimasafish":  return "Swim As A Fish";
    case "windrush":     return "Wind Rush";
    case "remoteview":   return "Remote View";
    case "mindblock":    return "Mind Block";
    case "objectread":   return "Object Read";
    case "readaura":     return "Read Aura";
    case "senseevil":    return "Sense Evil";
    case "psisword":     return "Psi-Sword";
    }
    words = explode(lower_case(name), " ");
    out = "";
    for(i = 0; i < sizeof(words); i++) {
        if(!strlen(words[i])) continue;
        if(strlen(out)) out += " ";
