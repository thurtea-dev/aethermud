...
string normalize_spell_name(string name) {
    ...
    case "magicnet":
        return "magic net";
    /* Spaced forms of the two collapsed canonical keys. Added 2026-07-26
       alongside RIFTS_D->display_ability_name(), which now shows these as
       "Swim As A Fish" and "Wind Rush" -- a player typing back what they
       were shown has to resolve. The canonical keys themselves are
       unchanged ("swimasafish" / "windrush" still cast). */
    case "swim as a fish":
    case "swimasfish":
        return "swimasafish";
    case "wind rush":
...
mapping query_spell(string name) {
    ...
    case "magic net":
        return ([ "ppe_cost":7, "duration":120, "range":"single",
                  "effect":"net",
                  "desc":"Snares target in a magic net (30 MDC). Cannot move or fight back, but remains vulnerable to damage. Save vs magic (14+) negates." ]);
    case "chameleon":
        return ([ "ppe_cost":6, "duration":600, "range":"self",
                  "effect":"chameleon",
    ...
    case "constrain being":
        return ([ "ppe_cost":10, "duration":60, "range":"single",
                  ...
                  "desc":"Magically bind target. Cannot move or fight back, but remains vulnerable to damage. Save vs magic (14+)." ]);
    case "astral projection":
...
private void fx_constrain_being(object target) {
    ...
    tell_object(target,
        "Magical bonds hold you fast! You cannot move or fight back,\n"
        "but you remain vulnerable to attack.\n");
...
private void fx_freeze_spell(object target) {
    ...
    if(roll < 16) {
        ...
        tell_object(target,
            "Ice encases your limbs! You cannot move or fight back,\n"
            "but you remain vulnerable to attack.\n");
...
private void write_spell_line(string name, mapping sdata, string tag, int verbose) {
    ...
    label = (string)RIFTS_D->display_ability_name(name);
...
private void write_spell_group(string *names, int verbose) {
    ...
    labels = ({});
    for(i = 0; i < sizeof(names); i++)
        labels += ({ (string)RIFTS_D->display_ability_name(names[i]) });
