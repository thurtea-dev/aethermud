...
/* Wield (weapons) or wear (armour) an item at the moment it's granted, so
   query_auto_load() (std/weapon.c, std/armour.c) picks it up and it survives
   quit. query_name() always self-matches id() (std/Object.c query_id() always
   appends query_name()), so this never falls through to parse_objects(). Any
   failure (e.g. no free hands left for a second two-handed weapon) just
   leaves the item as loose carried inventory, same as before this fix. */
private void equip_granted_item(object player, object ob) {
    string nom;

    if(!player || !objectp(player) || !ob || !objectp(ob)) return;
    if(environment(ob) != player) return;
    nom = (string)ob->query_name();
    if(!nom || !strlen(nom)) return;
    if((int)ob->is_weapon()) {
        if(ob->query_wielded()) return;
        catch(ob->cmd_wield(nom));
    }
    else if((int)ob->is_armour()) {
...
private void give_equip(object player, string occ) {
    ...
    case "cs military specialist":
        /* occ.c: "Dead Boy armor, C-20 laser pistol, C-12 heavy assault
           rifle, four E-clips..." Previously shared CS Ranger's case
           (grunt armor, CP-40 rifle, C-18 pistol, 1 e-clip) -- wrong
           armor, wrong rifle, wrong clip count. cp30_laser.c stands in
           for the C-20 (no distinct item exists), same substitution used
           for CS SAMAS RPA Pilot's C-20. */
        give_item(player, "dead_boy_armor.c");
        give_item(player, "cp30_laser.c");
        give_item(player, "c12_laser_rifle.c");
        give_item(player, "e_clip.c");
    ...
    case "master assassin":
        /* occ.c: "Concealed armor, three exotic weapons..." Previously 2
           identical vibro-knives and no ranged weapon, despite the W.P.
           Two Modern base skill. */
        give_item(player, "vibro_knife.c");
    ...
    case "sunaj assassin":
        /* occ.c: "Atlantean combat armor..., concealed blade and two
           sidearms..." Previously shared Master Assassin's case (2
           identical knives, no ranged). */
        give_item(player, "vibro_knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "cp30_laser.c");
        give_item(player, "e_clip.c");
    ...
    case "kankoran":
        /* occ.c: "Hunting rifle or shotgun, hunting knife..." Previously
           no rifle at all despite the W.P. Rifle or Shotgun base skill.
           hunting_rifle.c is new (see file header for the ammo-type note). */
        give_item(player, "hunting_rifle.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
        give_item(player, "vibro_knife.c");
        give_item(player, "personal_radio.c");
    ...
    /* ── Tribal Warrior ──────────────────────────────────────────────────────── */
    case "tribal warrior":
        /* occ.c: "...two ancient weapons, hunting rifle..." Previously one
           melee weapon (obsidian_blade) and no rifle despite the W.P.
           Rifle base skill. */
    ...
    /* ── Vagabond ───────────────────────────────────────────────────────────── */
    case "vagabond":
        /* occ.c: "...and one ranged weapon of choice." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── City Rat ───────────────────────────────────────────────────────────── */
    case "city rat":
        /* occ.c: "...laser pistol or revolver..." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── Rogue Scholar ───────────────────────────────────────────────────────── */
    case "rogue scholar":
        /* occ.c: "...concealed sidearm..." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    case "cs technical officer":
        /* cs technical officer already received c18_pistol.c here before
           this pass (audit's "no weapon" note on this OCC was stale) --
           only the spare e-clip was missing. */
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
        give_item(player, "rations.c");
    ...
    /* ── Shaman ──────────────────────────────────────────────────────────────── */
    case "shaman":
        /* occ.c: "...ceremonial staff or weapon..." No ranged weapon
           named -- unlike Air Warlock (split out below), kept melee-only. */
        give_item(player, "knife.c");
        give_item(player, "leather_jacket.c");
        give_credits(player, (random(6)+1) * 100);
        break;
    ...
    /* ── Air Warlock ─────────────────────────────────────────────────────────── */
    case "air warlock":
        /* occ.c: "...and a concealed sidearm." Previously shared Shaman's
           melee-only case despite differing text. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── Mystic ──────────────────────────────────────────────────────────────── */
    case "mystic":
        /* occ.c: "...and one concealed ranged weapon." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── Shifter ─────────────────────────────────────────────────────────────── */
    case "shifter":
        /* occ.c: "...and a sidearm." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── Cyber-Doc ───────────────────────────────────────────────────────────── */
    case "cyber-doc":
        /* occ.c: "...personal sidearm..." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    /* ── Gunfighter ─────────────────────────────────────────────────────────── */
    case "gunfighter":
        /* occ.c: "Twin C-18 laser pistols..." Previously zero spare
           e-clips for either signature pistol. */
        give_item(player, "c18_pistol.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "e_clip.c");
    ...
    /* ── Necromancer ────────────────────────────────────────────────────────── */
    case "necromancer":
        /* occ.c: "...one sidearm..." Previously melee-only. */
        give_item(player, "knife.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
        give_item(player, "leather_jacket.c");
    ...
    case "borg":
        ...
        /* occ.c text: "two integrated ranged weapons, vibro-blade, and
           maintenance toolkit" -- matches the W.P. Energy Pistol / W.P.
           Energy Rifle base skills. full_borg_conversion only sets stats
           (bionic_d.c), it never grants any weapon, so this OCC previously
           received zero weapons of any kind. */
        give_item(player, "c18_pistol.c");
        give_item(player, "c12_laser_rifle.c");
        give_item(player, "e_clip.c");
        give_item(player, "e_clip.c");
    ...
    /* ── Atlantean Tattoo Warrior / Nomad / Tattooed Man ─────────────────────
       Previously one shared case granting 2 e-clips with no energy weapon
       to use them in, for all three. Split per each OCC's own occ.c
       starting_equipment text: */
    case "tattoo warrior":
        /* occ.c: "...two energy clips, and four magical tattoos..." --
           text explicitly wants the clips; it just never named the weapon
           they belong in, so give a compact sidearm to match. */
        give_item(player, "vibro_sword.c");
        give_item(player, "environmental_body_armor.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
    ...
    case "tattooed man":
        /* occ.c: "...ancient weapon tied to a tattoo, one ranged weapon." */
        give_item(player, "vibro_sword.c");
        give_item(player, "environmental_body_armor.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "e_clip.c");
    ...
    case "atlantean nomad":
        /* occ.c: "...a short sword with at least one magic tattoo weapon
           effect, dimensional compass, and survival gear." No ranged
           weapon and no energy clips in its own text -- previously got 2
           dead e-clips anyway. */
        give_item(player, "vibro_sword.c");
        give_item(player, "environmental_body_armor.c");
        give_credits(player, (random(4)+1 + random(4)+1) * 100);
    ...
    case "cs samas rpa pilot":
        ...
        give_item(player, "cp30_laser.c");
        /* occ.c text names "C-20 laser pistol, TX-5 particle beam rifle" --
           the rifle never had an item file until this fix (tx5_particle_
           rifle.c, new). cp30_laser.c stands in for the C-20 (no distinct
           C-20 item exists), matching the sidearm-substitution pattern
           already used elsewhere in this file. */
        give_item(player, "tx5_particle_rifle.c");
        give_item(player, "e_clip.c");
        give_item(player, "e_clip.c");
        give_item(player, "military_radio.c");
        give_credits(player, (random(6)+1 + random(6)+1) * 100);
    ...
    case "pirate (s.a.)":
    case "pirate":
        /* occ.c: "...two ranged weapons, cutlass or vibro-blade..."
           Previously melee-only (vibro_sword + knife), no ranged weapon
           at all despite the W.P. Two Modern base skill. Dropped the
           plain knife -- text calls for one blade, not two. */
        give_item(player, "vibro_sword.c");
        give_item(player, "c18_pistol.c");
        give_item(player, "cp30_laser.c");
        give_item(player, "e_clip.c");
