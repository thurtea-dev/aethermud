...
int cmd_mend(string str) {
    ...
    /* Unlike _heal.c/_cure.c, this never had a skill floor, so the wisdom
       stat alone produced a real heal for anyone regardless of skill.
       Matching their existing floor (2026-07-29). */
    if(healing < 5) {
