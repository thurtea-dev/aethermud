...
int cmd_discern(string str) {
    ...
    if(!x) {
	notify_fail("That is neither armor or a weapon!\n");
        return 0;
    }
    if(!ob->query_wc()) {
	if(!limbs=ob->query_limbs())
            return notify_fail("That is neither armor or a weapon!\n");
	tp->add_sp(-10);
...
void help() {
    message("help", "Syntax: <discern [weapon | armor]>\n\n"
      "For fighters, this gives what type of weapon or armor the "
      "item is.  If it is armor, it tells you what it protects.",
      this_player());
}
