...
int cmd_bump(string str) {
    ...
    /* No throttle at all before this, unlike sibling files in this
       directory (_aim.c, _lockpick.c, _parry.c) - added the same
       disable/cooldown pattern so this can't be spammed (2026-07-29). */
    ...
    sorties = (string *)environment(ob)->query_exits();
   ...
   this_player()->add_sp(-(random(10)));
    ...
    newroom = (string)environment(ob)->query_exit(sorties[random(sizeof(sorties))]);
    ob->move(newroom);
