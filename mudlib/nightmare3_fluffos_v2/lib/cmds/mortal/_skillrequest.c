...
int cmd_skillrequest(string str) {
    ...
    if(wiz_online > 0) {
        ...
        /* unguarded: /secure/save/ is write-restricted to ASSIST by
           secure/cfg/write.cfg, and master.c check_access() walks the whole
           call stack -- a mortal invoking this command is in that stack, so
           a bare write_file() was silently denied and the request was never
           saved. Same root cause as the account_d.c lookup bug. */
        if(!unguarded((: write_file, fname,
            "player:" + pname + "\n" +
            "text:" + skill_name + "\n" +
            "time:" + time() + "\n" :))) {
            write("Your request could not be saved. Please tell staff.\n");
            return 1;
        }
