...
void look_msg(object ob, string str) {
  ...
  who = this_player();
  who_name = (string)who->query_display_name(ob);
  if( ob != who )
    if(living(ob)) {
      ob_name = (string)ob->query_display_name(who);
      say(who_name + " looks over " + ob_name + ".", ob);
      tell_object(ob, who_name + " looks you over.");
    }
    else say(who_name + " looks over the " + str + ".");
}
