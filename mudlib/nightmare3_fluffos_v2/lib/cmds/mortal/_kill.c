...
int cmd_kill(string str) {
   ...
   tell_object(victim,
     (string)this_player()->query_display_name(victim) +
     " attacks you!\n");
   obs = all_inventory(environment(this_player()));
   for(i = 0; i < sizeof(obs); i++) {
       if(!obs[i] || obs[i] == this_player() || obs[i] == victim) continue;
       att_name = (string)this_player()->query_display_name(obs[i]);
       vic_name = (string)victim->query_display_name(obs[i]);
       tell_object(obs[i], att_name + " attacks " + vic_name + "!\n");
   }
   return retvalue;
}
