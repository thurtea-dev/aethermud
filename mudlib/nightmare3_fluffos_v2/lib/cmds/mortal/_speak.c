...
void tell_em(object speaker, string tail, object *ob);
...
int cmd_speak(string str)
{
   ...
   /* NPCs bypass language filtering: always speak plainly to the room. */
...
void speak(string lang, string str, int prof)
{
   ...
   if(speech[10])
     tell_em(this_player(), " says in " + cap_lang + ": " + str, speech[10]);
   if(speech[0])
     tell_em(this_player(), " says something in " + cap_lang + ".", speech[0]);
   for(i = 1; i < 10; i++)
     if(speech[i])
       tell_em(this_player(), " says in " + cap_lang + ": " +
...
void tell_em(object speaker, string tail, object *ob) {
   ...
   for(i = 0, obsz = sizeof(ob); i < obsz; i++) {
    pname = (string)speaker->query_display_name(ob[i]);
    message("talk", pname + tail, ob[i]);
   }
}
