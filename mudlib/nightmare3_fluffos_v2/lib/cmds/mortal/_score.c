...
private void show_header(object who) {
    ...
    /* Held form appended rather than substituted: the sheet keeps showing
       the character's true race, with the disguise noted alongside it. */
    if((int)who->query_property("metamorphed")) {
        held_form = (string)who->query_property("apparent_race");
        if(!held_form || held_form == "")
            held_form = (string)who->query_property("current_form");
        if(held_form && held_form != "" &&
           lower_case(held_form) != lower_case((string)who->query_race()))
            race = race + " (as " + tc(held_form) + ")";
    }
