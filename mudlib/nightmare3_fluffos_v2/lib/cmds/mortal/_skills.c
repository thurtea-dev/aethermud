...
private string compact_skill_label(string sname) {
    ...
    s = (string)RIFTS_D->display_ability_name(sname);
...
static void show_standard_skills() {
    string *attack, *weapon, *decep;
    int i;

    attack = ({ "melee", "attack", "defense", "double wielding" });
    weapon = ({ "knife", "blade", "blunt", "two handed", "projectile" });
    /* stealth/stealing removed (2026-07-29): neither exists in the
       Rifts skill system (daemon/rifts_skills.c), so they always showed
       a flat 0% for every character. locks/murder/bargaining are
       unaffected, still real query_base_skill() lookups. The former
       "Magic" column (faith/healing/conjuring/magic attack/magic
       defense) is dropped outright rather than left half-populated -
       all five were equally dead, so there was nothing left worth
       keeping in that category. */
    decep  = ({ "locks", "murder", "bargaining" });
    ...
    for(i = 0; i < 5; i++) {
        ...
        if(i < 3) {
            ...
            message("Ninfo", arrange_string(
                "" + this_player()->query_base_skill(decep[i]), 4) + "\n", this_player());
        } else {
            message("Ninfo", "\n", this_player());
        }
    }
