...
private int is_groups_cfg_member(string name) {
    ...
    data = (string)unguarded((: read_file, "/secure/cfg/groups.cfg" :));
...
private int get_saved_level(string name) {
    ...
    data = (string)unguarded((: read_file, save_path(name) :));
    if(!data) return -1;
    lines = explode(data, "\n");
    for(i = 0; i < sizeof(lines); i++) {
...
private void rm_tree(string dir) {
    ...
    if((int)unguarded((: file_size, dir :)) != -2) return;
    files = (string *)unguarded((: get_dir, dir :));
    if(!files) return;
    for(i = 0; i < sizeof(files); i++) {
        if(files[i] == "." || files[i] == "..") continue;
        full = dir + files[i];
        if((int)unguarded((: file_size, full :)) == -2)
            rm_tree(full + "/");
        else
            unguarded((: rm, full :));
    }
...
private void wipe_one(string name) {
    ...
    if((int)unguarded((: file_size, path :)) >= 0) unguarded((: rm, path :));
    if((int)unguarded((: file_size, bak :)) >= 0) unguarded((: rm, bak :));
    if((int)unguarded((: file_size, postal :)) == -2) rm_tree(postal);
    if((int)unguarded((: file_size, realm :)) == -2) rm_tree(realm);
...
int cmd_playerwipe(string str) {
    ...
    if(str == "confirm all") {
        wiped = 0;
        letters = (string *)unguarded((: get_dir, "/secure/save/users/" :));
        if(!letters) {
        ...
        for(i = 0; i < sizeof(letters); i++) {
            ...
            if((int)unguarded((: file_size, dir :)) != -2) continue;
            names = (string *)unguarded((: get_dir, dir :));
            if(!names) continue;
    ...
    if((int)unguarded((: file_size, save_path(name) :)) < 0) {
