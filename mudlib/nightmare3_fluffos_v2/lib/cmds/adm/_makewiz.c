...
/* Kept in sync with the identical list in cmds/adm/_setrole.c. A
   whotitle matching one of these is a leftover auto-default, not a
   custom title staff typed in themselves, so promotion is free to
   replace it. makewiz only ever targets a mortal (creatorp(target)
   above already refuses an existing wizard), so in practice whotitle
   is always unset here -- this check exists for symmetry with
   _setrole.c's reuse of the same helper name, not because a mortal
   can currently acquire a whotitle some other way. */
...
int cmd_makewiz(string str) {
    ...
    /* Default who-list flavor title. whotitle is a separate env from
       TITLE (finger/advance/combat's $N template) -- see cmds/mortal/
       _who.c format_wiz_row(). Only replaces an unset/leftover-default
       whotitle, so a custom title set some other way is never
       clobbered. */
    if(is_auto_whotitle((string)target->getenv("whotitle")))
        target->setenv("whotitle", "A NEW Apprentice Wizard!");
