...
void change_sky() {
    ...
    switch(__TimeOfDay) {
        case "night":
            __TimeOfDay = "dawn";
            msg = "The sun appears over the horizon of our reality.";
            break;
        case "dawn":
            __TimeOfDay = "day";
            msg = "The sun now shines completely on a new day.";
            break;
        case "day":
            __TimeOfDay = "twilight";
           msg = "The sun begins to fall away into twilight.";
            break;
        case "twilight":
            __TimeOfDay = "night";
    ...
    next_change();
    /* master()->valid_function() is a permanent stub (always returns 1,
       see secure/daemon/master.c), so it never actually screened out a
       closure bound to a destructed object -- e.g. supply2.c's
       "shop_closing" callback, re-registered on every room reload with
       nothing ever calling remove_sky_event() on the old one. catch()'s
       own return value (an error string on failure, 0 on success) is a
       real validity signal, so use that to prune dead entries instead. */
    i = sizeof(__TimeChange);
    while(i--)
      if(catch((*__TimeChange[i])(__TimeOfDay)))
        __TimeChange -= ({ __TimeChange[i] });
}
