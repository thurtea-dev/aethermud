...
string user_finger_display(string who) {
    ...
    name = lower_case(who);
    if(!user_exists(name)) return "No such member of AetherMUD.";
    ...
    if(!race=query_race())
        race = "unborn";
    /* vampires appear human to non-arch viewers; aura-reading reveals truth */
    if(strsrch(lower_case(race), "vampire") != -1)
