...
object compile_object(string str) {
    ...
    if(sscanf(str, REALMS_DIRS+"/%s/%*s", nom)){
      ...
      tmp = sprintf("%s/adm/server", ESTATES_DIRS);
        ...
        /* query_name() is the ACCOUNT name since the 2026-07-18
           account-separate login split (secure/std/login.c); it no
           longer equals the character name being created/selected
           except by coincidence (a fresh account's first character,
           whose name is seeded to match the account name). Use
           query_char_name() instead, which login.c keeps in sync
           with "nom" before ever calling player_object(). */
        if((string)__NewPlayer->query_char_name() != nom) return 0;
