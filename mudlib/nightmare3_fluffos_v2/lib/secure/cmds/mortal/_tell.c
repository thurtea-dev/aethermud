...
int cmd_tell(string str) {
    ...
    if(!mud || mud == -1) {
        ...
        /* Intermud tells are staff-only. Without this gate a mortal could
           reach an off-mud player via "tell who@mud msg", bypassing the
           staff-only check below (which only runs when ob is set, i.e. on
           the local-target path). See CLAUDE.md rule 20. */
        if(!creatorp(this_player())) {
            write("Intermud tells are for staff only.\n");
            return 1;
        }
    ...
    if(ob) {
        ...
        if(!creatorp(this_player()) && !creatorp(ob)) {
            write("Tell is for contacting staff only. "
...
void help() {
    message("help",
      ...
      "Players may use tell to contact online staff only.\n"
      "To reach other players, use telepathy after introducing yourself.\n"
      "Staff may tell anyone on this mud or on another mud when a mud name "
      "is specified.  For muds with more than one word in their names, use "
      ". (periods) instead of spaces.  Example: tell "
      "descartes@realms.of.chaos hi\n\n"
      "See also: telepathy, introduce, say, shout, yell, emote", this_player()
    );
}
