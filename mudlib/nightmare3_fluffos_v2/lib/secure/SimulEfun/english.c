...
#define ABNORMAL ([ "moose":"moose", "mouse":"mice", "die":"dice", "index":"indices", "human":"humans", "sheep":"sheep", "fish":"fish", "child":"children", "ox":"oxen", "tooth":"teeth", "deer":"deer", "bus":"busses", "rations":"rations" ])
...
string pluralize(mixed single) {
    ...
    x = strlen(str);
    if(ABNORMAL[str]) {
        /* Preserve surrounding words, same as every branch below --
           this used to return the bare replacement alone, dropping
           any other words in a multi-word name ("field rations"
           would have come back as just "rations"). */
        words[ind] = ABNORMAL[str];
