...
private string display_vendor_type(string t);
private string article_for(string t);
    ...
    if(!(ob->query_property("magic item") &&
	 member_array("magic", query_item_types()) != -1)) {
      this_object()->force_me(sprintf("speak I do not buy %s goods.",
				      display_vendor_type((string)ob->query_vendor_type())));
      return 1;
    }
  }
...
static int __SellAll() {
  ...
  while(i--) {
    ...
    if(member_array((string)inv[i]->query_vendor_type(),
		    query_item_types()) == -1) {
      vtype = display_vendor_type((string)inv[i]->query_vendor_type());
      this_object()->force_me(sprintf("speak %s is %s %s item, and "
				      "I do not buy those.",
...
/* Display-only spelling fix for the vendor-type word shown to the
   player in refusal messages. Does not touch the stored vendor-type
   value itself (still "armour", set by /std/armour.c, which is never
   altered) or any of the internal comparisons against it -
   query_item_types()/__SellAll() above still match "armor" and
   "armour" both, unaffected by this. */
private string display_vendor_type(string t) {
    if(t == "armour") return "armor";
    return t;
}

/* Picks "a" or "an" based on the first letter of the word that will
   actually be displayed (after display_vendor_type() has already
   substituted "armour" -> "armor", etc), not the raw vendor-type
   string. */
private string article_for(string t) {
    if(!stringp(t) || !strlen(t)) return "a";
    switch(t[0]) {
