...
int cmd_wield(string str) {
    ...
    player = this_player();
    hand_given = (sscanf(str, "%s in %s", what, hand1) == 2);
    if(!hand_given)
        what = str;
    if(!id(what)) {
        if(!(ob = parse_objects(player, what))) return 0;
    ...
    if(query("skill level") > (int)player->query_skill(query_type()))
    ...
    if(!hand_given) {
        hand1 = "";
        hand2 = 0;
        wlimbs = (string *)player->query_wielding_limbs();
        if(__Weapon["type"] == "two handed") {
            for(i = 0; i < sizeof(wlimbs); i++) {
                if(player->query_weapon(wlimbs[i])) continue;
                if(!hand1)
                    hand1 = wlimbs[i];
                else {
                    hand2 = wlimbs[i];
            ...
            if(!hand1 || !hand2) {
                notify_fail("You have no free hands.\n");
                return 0;
            }
        } else {
            for(i = 0; i < sizeof(wlimbs); i++) {
    ...
    if(__Weapon["type"] == "two handed") {
        ...
        ok = (string)player->equip_weapon_to_limb(this_object(), hand1,
          hand2);
    }
    else {
        ...
        if(functionp(__Weapon["wield"]) && !((*__Weapon["wield"])())) return 1;
        ok  = (string)player->equip_weapon_to_limb(this_object(),hand1,0);
    }
    if(ok) {
        message("my_action", ok, player);
        return 1;
    }
    if(stringp(__Weapon["wield"]))
      message("my_action", __Weapon["wield"], player);
    else message("my_action", "You wield "+query_short()+".", player);
    message("other_action", (string)player->query_cap_name() +
      " wields " + query_short() + ".", environment(player),
      ({ player }));
    __Weapon["wielded"] = player;
...
/* Stopgap for the universal item-drop-on-quit problem: ordinary
   weapons have no query_auto_load() at all, so quit()'s teardown
   chain (std/user.c remove() -> ... -> std/clean_up.c remove()) dumps
   them into whatever room the player quit in, wielded or not, since
   nothing else preserves them. Rather than an opt-in allowlist per
   item file, this default applies automatically to every WEAPON
   subclass: currently-wielded means "worth restoring," gated
   dynamically on query_wielded() so it self-updates the moment a
   weapon is un/rewielded, with no separate flag to maintain.
   pre_save() (std/user/autosave.c) runs at the START of quit(),
   before this cascade unwields/drops anything, so a wielded weapon is
   captured into __AutoLoad while still wielded. std/user.c's own
   remove() then runs autosave::remove() BEFORE living::remove(), and
   autosave::remove() calls ->remove() directly on every autoload-
   tagged item still in inventory -- which for a weapon (remove() ==
   unequip() + ::remove(), no move() involved) cleanly unwields and
   destructs it with no floor-drop, before living.c's own drop-gated
   loop or clean_up.c's move-everything-to-the-room dump ever see it.
   Loose (unwielded) inventory is unaffected and still drops as
   before -- this only covers what's actively equipped. */
mixed *query_auto_load() {
    if(!query_wielded()) return 0;
    return ({ base_name(this_object()), ({}) });
}

/* Runs once the restored weapon is already back in the player's
   inventory (autosave.c moves it in before calling init_arg()).
   cmd_wield() reads this_player() internally the same way
   ring_of_dominion.c's own init_arg() already relies on this_player()
   being correctly bound during this exact restore sequence.
   query_name() is normally one of the weapon's own ids, matching what
   a player would type by hand ("wield <name>"); if wielding fails for
   any reason (no free hands, skill too low now, etc) the weapon is
   simply left unwielded in inventory rather than lost. */
void init_arg(mixed *arg) {
    catch(cmd_wield(query_name()));
}
