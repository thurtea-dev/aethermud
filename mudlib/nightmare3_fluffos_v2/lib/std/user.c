...
/* Additive: name-based matching (__UserId) is unchanged above. Below,
   also match the looker's race/RCC word, e.g. "look at great horned
   dragon" for a Great Horned Dragon player, exactly like "look at
   percivil" already works for them by name. Uses query_apparent_race()
   (this_player() as viewer) rather than query_race() directly, so a
   metamorphed/disguised player is only id()-matchable by the race word
   a stranger would actually perceive -- the same precedence already
   used for the "You look over the ..." race line, not the true race. */
int id(string str) {
    string race_name;

    if(!str) return 0;
    if(member_array(str, __UserId) != -1) return 1;
    race_name = query_apparent_race(this_player());
    if(race_name && race_name != "" && lower_case(race_name) == lower_case(str))
        return 1;
    /* The character themselves and staff additionally match the held form
       word, so a disguised character (or a wizard checking one) can target
       the disguise by name -- query_apparent_race() hands those two viewers
       the TRUE race, so without this "look at human" fails for exactly the
       people who need to verify the disguise took effect.
       Deliberately NOT extended to strangers: a stranger matching both the
       held form AND the true race would be able to confirm a disguise just
       by trying the true race word. */
    if(this_player() && (int)query_property("metamorphed") &&
       (this_player() == this_object() || creatorp(this_player()))) {
        race_name = (string)query_property("apparent_race");
        if(!race_name || race_name == "")
            race_name = (string)query_property("current_form");
        if(race_name && race_name != "" &&
           lower_case(race_name) == lower_case(str))
            return 1;
    }
    return 0;
}
...
/* Driver apply hook (see comm.c APPLY_WINDOW_SIZE): fires when the
   client answers NAWS telnet negotiation, on connect and again on any
   live window resize. Stored separately from the player-settable
   LINES so `setenv LINES <n>` always wins over auto-detection; more.c
   falls back to this only when LINES was never set manually. */
void window_size(int width, int height) {
    if(height < 3 || height > 500) return;
