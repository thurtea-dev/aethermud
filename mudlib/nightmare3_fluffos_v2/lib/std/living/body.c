...
void augment_body(int x) {
    ...
    if(b_type != 0)   // must be a monster  added by Val June 30, 1993
    {
      for(i=0, tmp = sizeof(limbs); i<tmp; i++) {
        /* whole_body is a user.c pseudo-limb, not in any RACE_D limb
           table; the table lookup returns 0 for it (division by zero).
           Keep its new_body() semantics: never fatal from limb damage. */
        if(limbs[i] == "whole_body") {
          body[limbs[i]]["max_dam"] = query_max_hp() + 1;
          continue;
        }
        div = (int)RACE_D->query_max_mon_dam(limbs[i], b_type);
        if(div < 1) div = 2;
        body[limbs[i]]["max_dam"] = query_max_hp()/div;
      }
      return;
    }
    for(i=0, tmp = sizeof(limbs); i<tmp; i++) {
      ...
      div = (int)RACE_D->query_max_dam(limbs[i],
