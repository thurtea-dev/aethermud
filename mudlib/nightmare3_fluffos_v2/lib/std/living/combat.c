...
int kill_ob(object victim, int which) {
    ...
    if(member_array(this_object(), borg_people) == -1)
        if(!ok_to_kill(victim)) {
            message("my_combat", sprintf("Supernatural forces prevent you "
              "from attacking %s.", id_name(victim, this_object())),
              this_object());
           return 0;
        }
    ...
    if(!which) {
        ...
        /* victim just got ambushed by a new attacker; check detect ambush */
        da_skill = (int)this_object()->query_skill("detect ambush");
        if(da_skill > 0) {
            da_roll = random(100) + 1;
            if(da_roll <= da_skill)
                message("my_combat",
                    "Your senses flare! You detect " +
                    id_name(victim, this_object()) +
                    " moving to attack! (Detect Ambush: " +
                    da_roll + "/" + da_skill + ")",
                    this_object());
        }
    }
...
void continue_attack() {
    ...
    } else if((int)this_object()->query_property("entangled") ||
              (int)this_object()->query_property("cannot_move")) {
        /* Bound (magic net, constrain being, freeze): cannot act, but */
        /* damage still lands on them via the normal combat path. */
        message("my_combat",
            "You are bound and cannot fight back!", this_object());
    } else {
        execute_attack();
    }
}
...
void execute_attack() {
    ...
    is_rifts_attacker = (int)RIFTS_D->is_rifts_race((string)this_object()->query_race());

    // Rifts APM: check that this character still has attacks left this melee round.
    if(is_rifts_attacker && !(int)RIFTS_COMBAT_D->can_do_attack(this_object())) return;

    if(casting) num_attacks = 1;
    else {
        if(is_rifts_attacker) {
            // Rifts characters make 1 attack per heart_beat, capped by APM per round.
            num_attacks = 1;
    ...
    if(is_rifts_attacker) RIFTS_COMBAT_D->use_rifts_attack(this_object());
    ...
    for(i=0; i<num_attacks; i++) {
        ...
        if(current && (int)current->query_property("entanglement") &&
           attackers[0] && living(attackers[0]))
            catch(RIFTS_COMBAT_D->try_entanglement_hit(
                this_object(), attackers[0], current));
        ...
        // Magic net: a trapped attacker's own outgoing attack is spent
        // against the net holding it, not against its target, until the
        // net breaks. Once broken, magic_net_obj is cleared and normal
        // attack flow resumes on its own.
        net = (object)this_object()->query_property("magic_net_obj");
        if(net && objectp(net) && damage > 0) {
            message("my_combat",
                "Your attack is absorbed by the glowing net around you!",
                this_object());
            message("my_combat",
                id_name(this_object(), attackers[0]) +
                "'s attack is absorbed by the net holding it.",
                attackers[0]);
            catch(net->take_net_damage(damage));
            x = -1;
        }
        // Route damage through Rifts chain if either combatant is a Rifts race.
        else if(damage > 0 &&
           (is_rifts_attacker ||
            (int)RIFTS_D->is_rifts_race((string)attackers[0]->query_race()))) {
            x = (int)RIFTS_COMBAT_D->apply_rifts_damage(
        ...
        if(!net && current && (pois = (int)current->query_poisoning()) &&
          damage > random(101)) {
...
int get_damage(object weap) {
    ...
    if(!weap) {
        if((int)this_object()->query_property("psi_sword_active"))
...
void send_messages(int magic, object current, string what, int x) {
    ...
    your_name = id_name(attackers[0], this_object());
    my_name = id_name(this_object(), attackers[0]);
    if(magic) type = "magic";
    ...
    if(x < 1) {
      ...
      me = sprintf("You %s %s%swith your %s.", verb[i], your_name,
        (adverb[i] ? " "+adverb[i]+" " : " "), used);
      you = sprintf("%s %ss you%swith %s %s.", my_name, verb[i],
        (adverb[i] ? " "+adverb[i]+" " : " "),
        (string)this_object()->query_possessive(), used);
      others = sprintf("%s %ss %s%swith %s %s.", my_name, verb[i],
        your_name, (adverb[i] ? " "+adverb[i]+" " : " "),
        (string)this_object()->query_possessive(), used);
    }
    ...
    for(j = 0; j < sizeof(obs); j++) {
        if(!obs[j] || obs[j] == this_object() || obs[j] == attackers[0]) continue;
        tell_object(obs[j],
            replace_string(replace_string(others,
                my_name, id_name(this_object(), obs[j])),
                your_name, id_name(attackers[0], obs[j])) + "\n");
    }
}
...
void miss(int magic, string type, string target_thing) {
    ...
    you = id_name(attackers[0], this_object());
    me = id_name(this_object(), attackers[0]);
    if(magic) {
        ...
        obs = all_inventory(environment(this_object()));
        for(j = 0; j < sizeof(obs); j++) {
            if(!obs[j] || obs[j] == this_object() ||
              obs[j] == attackers[0]) continue;
            att_disp = id_name(this_object(), obs[j]);
            def_disp = id_name(attackers[0], obs[j]);
            tell_object(obs[j], sprintf("%s's magic powers fail in an attack "
              "on %s.\n", att_disp, def_disp));
        }
        return;
    }
    missed = ({
      ...
      "Your attack goes wide, hitting nothing but air.",
    ...
    for(j = 0; j < sizeof(obs); j++) {
        ...
        att_disp = id_name(this_object(), obs[j]);
        def_disp = id_name(attackers[0], obs[j]);
        other_msg = sprintf("%s misses %s attack on %s.",
            att_disp, (string)this_object()->query_possessive(), def_disp);
        tell_object(obs[j], other_msg + "\n");
    }
