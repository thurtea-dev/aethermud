...
void create() {
    ...
    init_skills(0);
    init_living();
    /* Default humanoid body so monsters that never call set_body_type()
       still have limbs; combat targeting (return_target_limb) indexes
       an empty limb array otherwise. A later set_body_type() rebuilds. */
    set_body_type("human");
...
void heart_beat() {
   ...
   ok_to_heal ++;
    ...
    race = __Race;
    if(race && (int)RIFTS_D->is_rifts_race(race) && query_current_attacker())
        catch(RIFTS_COMBAT_D->npc_combat_tick(this_object()));
   if(!(int)RIFTS_COMBAT_D->npc_round_active(this_object())) continue_attack();
...
void die(object ob) {
   ...
   int i, tmp_size;
    ...
    if(functionp(__Die)) {
      ...
      message("other_action",__Die,env,({this_object()}));
    else message("other_action", query_cap_name()+
      " drops %^RED%^dead%^RESET%^ before you.", env,
      ({ this_object() }));
    tmp = new(OB_CORPSE);
    ...
    tmp->move(env);
    if(env)
        catch(RIFTS_D->drop_death_blood(this_object(), env));
   ...
   /* Alignment shift: find the player who landed the kill. The combat
      system calls die() with no argument, so fall back to current attacker. */
   killer = (object)query_current_attacker();
   if(!killer) killer = ob;
   if(killer && (int)interactive(killer))
       catch(RIFTS_COMBAT_D->npc_died(this_object(), killer));
   remove();
}
