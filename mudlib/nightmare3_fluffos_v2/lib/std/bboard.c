...
void init() {
    ...
    /* "respond" had a trailing space here, so the verb could never match
       and the respond branch in cmd_followup_and_respond() (which tests
       query_verb() == "respond") was unreachable. Fixed 2026-07-27. */
    add_action("cmd_followup_and_respond", ({ "followup", "respond" }));
    add_action("cmd_remove", "remove");
    add_action("cmd_edit", "edit");
    /* Removed 2026-07-27: add_action("cmd_save", "save") for creators.
       cmd_save() does not exist anywhere in this inherit chain and never
       has, so the action could only ever fail. Worse, add_action always
       beats a directory-searched command, so it shadowed the real
       cmds/mortal/_save.c for any creator standing near a board. That was
       nearly unreachable while the only board was the legacy approval
       room's, but staff boards now sit in the Wizard's Lounge, the Admin
       Chamber, and admin workrooms, where every occupant is a creator.
       Do not re-add this without writing a cmd_save() to go with it. */
}
