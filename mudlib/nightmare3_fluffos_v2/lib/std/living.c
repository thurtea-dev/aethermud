...
static private int __LastRiftsRegen;
static private int __LastSunCheck;
int __PlayerKiller;
...
void heart_beat() {
    ...
    if(now - __LastRiftsRegen >= 3600) {
        __LastRiftsRegen = now;
        catch(RIFTS_COMBAT_D->do_rifts_regen(this_object()));
    }
    /* Sunlight exposure for sun-vulnerable races, one melee round apart
       (ROUND_DURATION in daemon/rifts_combat.c). All of the deciding logic
       lives in the daemon so it stays update-safe; only this gate needs a
       reboot. The daemon early-outs on the cheap checks first, so this
       costs almost nothing for the overwhelming majority of livings. */
    if(now - __LastSunCheck >= 6) {
        __LastSunCheck = now;
        catch(RIFTS_COMBAT_D->do_sunlight_exposure(this_object()));
    }
}
...
static void init_path() {
    ...
    /* QCS builder verbs (create/modify/add/delete/about/more) go ahead of
       DIR_HM_CMDS and DIR_AMBASSADOR_CMDS so they win the "create" name
       clash with the legacy hm conjuring command and the "more" name
       clash with the ambassador file pager, for the wizards who hold
       the staff of creation. */
    if(admin_wizp(this_object()) || coding_wizp(this_object()))
      search_path += ({ DIR_QCS_CMDS });
...
void set_stats(string str, int x) {
    ...
    if(stats[str] && stats[str] != x) {
        ...
        " in "+str+" ("+ctime(time())+")\n");
        if(objectp(prev))
            log_file("stats", "PRIV: "+query_privs(prev)+" ("+
            file_name(prev)+")\n");
    }
    stats[str] = x;
    if(str == "constitution") {
...
void add_exp(int x) {
    ...
    if(x > 0) catch(RIFTS_D->check_advance_notify(this_object()));
}
...
void tell_room_living(object env, object speaker, object exclude, string text) {
    ...
    if(!env || !speaker || !text) return;
    inv = all_inventory(env);
    for(i = 0; i < sizeof(inv); i++) {
        if(!inv[i] || !living(inv[i])) continue;
        if(inv[i] == speaker || inv[i] == exclude) continue;
        shown = (string)speaker->query_display_name(inv[i]);
        tell_object(inv[i], shown + text);
    }
}
...
string query_long(string unused) {
    ...
    viewer = this_player();
    shown = query_cap_name();
    if(viewer)
        shown = (string)query_display_name(viewer);
    ...
    /* Held-form indicator (2026-07-26). Shown only to the character
       themselves and to staff -- exactly the two viewers that
       query_apparent_race() already exempts from the disguise, so this
       cannot leak a disguise to a stranger who should be fooled by it.
       Without this line a metamorphed character had no way to confirm
       their own disguise was in effect. */
    if((int)this_object()->query_property("metamorphed") && viewer &&
       (viewer == this_object() || creatorp(viewer))) {
        form = (string)this_object()->query_property("apparent_race");
        if(!form || form == "")
            form = (string)this_object()->query_property("current_form");
        if(form && form != "" &&
           lower_case(form) != lower_case((string)this_object()->query_race()))
            pre += "Currently holding the form of a "+form+".\n";
    }
