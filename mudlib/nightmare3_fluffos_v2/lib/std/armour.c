...
/* Stopgap for the universal item-drop-on-quit problem (same reasoning
   as std/weapon.c's identical addition -- see that file for the full
   trace of why this alone is enough, with no clean_up.c/living.c
   changes needed). Applies automatically to every ARMOUR subclass:
   currently worn means "worth restoring," gated dynamically on
   query_worn() so it self-updates the moment a piece is worn/removed.
   Subclasses that already define their own query_auto_load() (the
   admin ring, wed_ring.c) are untouched -- their own definition
   already shadows this default. */
mixed *query_auto_load() {
    if(!query_worn()) return 0;
    return ({ base_name(this_object()), ({}) });
}

/* Mirrors ring_of_dominion.c's own init_arg(), generalized: wear()
   here matches this exact object's own id()/query_name() the same
   way a player typing "wear <name>" would, and covers both the
   protective (equip_armour_to_limb) and cosmetic-slot wear paths
   internally, so no per-subtype branching is needed here. If wearing
   fails for any reason (slot taken, no longer fits, etc) the piece is
   simply left unworn in inventory rather than lost. */
void init_arg(mixed *arg) {
    catch(wear(query_name()));
}
