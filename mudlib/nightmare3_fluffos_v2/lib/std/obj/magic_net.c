...
// Has MDC; victim is bound (cannot move or attack) but remains fully
// vulnerable to incoming damage until the net is destroyed or times out.
...
void timed_release() {
    release_victim("The magic net fades away and dissolves.\n");
}
...
int capture(object victim, object caster) {
    ...
    if(caster && room) {
        ...
        tell_object(victim,
            cshown + " snares you in a glowing magic net! You are bound\n"
            "and cannot attack or flee, but you remain vulnerable to attack.\n");
