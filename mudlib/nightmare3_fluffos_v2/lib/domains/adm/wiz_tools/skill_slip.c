...
void approve_get_pct(string pct_str) {
    ...
    if(target && objectp(target)) {
        ...
        /* unguarded: see _skillrequest.c -- /secure/save/ writes are
           ASSIST-only and the acting wizard is in the call stack. */
        if(!unguarded((: write_file, fname,
                        skill_name + ":" + pct + "\n" :))) {
            write("Grant could not be saved. Please tell staff.\n");
            return;
        }
