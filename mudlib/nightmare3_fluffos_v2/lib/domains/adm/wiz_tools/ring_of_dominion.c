...
/* Self-dedup: unlike the physical staves (WIZTOOLS_D->give_tools()),
   this ring is a one-off item placed by hand (see file header), so it
   never goes through give_tools()/ensure_wiz_tools()'s has_wiz_tools
   env flag or wiz_tools-substring inventory scan -- the exact
   machinery that stops the physical staves from being re-granted onto
   an inventory that already has them. Nothing currently clones a
   second ring automatically, but if one ever does turn up alongside
   an existing one (a stray manual clone, a future autoload edge case),
   this keeps whichever copy is actually worn and destructs the other,
   rather than leaving both sitting in inventory.
   Returns 1 if this copy just destructed itself as the surplus one
   (caller must not touch this_object() again after that), 0 if this
   copy survives (no duplicate found, or the other copy was the one
   destructed instead). */
private int dedup_ring() {
    object *inv;
    object other;
    ...
    for(i = 0; i < sizeof(inv); i++) {
        ...
        if(base_name(inv[i]) != base_name(this_object())) continue;
        other = inv[i];
        break;
    }
    if(!other) return 0;
    if(query_worn() == this_player()) {
        /* This copy is worn; the other one is the surplus. */
        if(other->query_worn() != this_player()) {
            tell_object(this_player(),
                "A duplicate gold ring dissolves back into nothing.\n");
            destruct(other);
        }
        return 0;
    }
    /* This copy is unworn; if the other is worn, this one is the
       surplus. If somehow neither is worn, still keep the older
       object (other) and drop this newer arrival. */
    tell_object(this_player(),
        "A duplicate gold ring dissolves back into nothing.\n");
