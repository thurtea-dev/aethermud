...
/* Direct one-line demote, bypassing the numbered menu only -- NOT the
   logic. This sets the same two pending_* slots menu choice 2 would set
   (pending_action + pending_sub, exactly as handle_choice() and
   get_target() do) and then calls the same do_player_action(), so an
   online demote, an offline USERS_D->offline_demote(), the not-a-wizard
   rejection, and the no-such-player rejection all behave identically to
   the menu path. Same admin_wizp() gate as begin_menu().

   Only demote gets this shortcut by design; promote and the higher-stakes
   options (set level/race/occ/stats, boot, purge, shutdown, reload) stay
   menu-only so they keep their deliberate extra prompt. */
int direct_demote(object player, string name) {
    if(!player || !objectp(player)) return 1;
    if(!admin_wizp(player)) {
        write("The staff refuses to respond to you.\n");
        return 1;
    }
    if(!name || !sizeof(name)) {
        write("Syntax: demote <wizardname>\n");
        return 1;
    }
    name = lower_case(name);
    if(strsrch(name, " ") != -1) {
        write("Syntax: demote <wizardname>  (one name, no extra words)\n");
        return 1;
    }
    pending_action[player] = "demote";
    pending_sub[player]    = name;
    do_player_action();
    /* Clear so a later menu session cannot inherit this target. */
    pending_action[player] = "";
    pending_sub[player]    = "";
    return 1;
}
...
/* Same validation setter.c's chargen race-picker uses: accept a race
   only if it's in the legacy race daemon's list or RIFTS_D->is_rifts_race()
   (which itself covers both the 62 real Rifts races and the explicit
   admin_races_list, currently just "wizard"). No allowlist bypass here -
   this is exactly how an unregistered value like "godling" got into a
   save file with no code path ever validating it (2026-07-29). */
void do_setrace(string race) {
    ...
    norm = lower_case(race);
    if(member_array(norm, (string *)RACE_D->query_races()) == -1 &&
       !(int)RIFTS_D->is_rifts_race(norm)) {
        write("\"" + race + "\" is not a supported race (not in the Rifts "
            "race list, the legacy race data, or admin_races_list). "
            "Race not changed.\n");
        return;
    }
    ob = find_player(pending_sub[player]);
...
/* Same shape as do_setrace()'s fix: mirror setter.c's chargen OCC check
   (OCC_D->query_occ(occ_name) must return real OCC data) rather than
   accepting any string. "none" is preserved as a valid special case -
   setter.c's own prompt text and chargen flow treat it as "no OCC,
   racial character only," not an actual OCC_D entry (2026-07-29). */
void do_setocc(string occ) {
    ...
    norm = lower_case(occ);
    if(norm != "none" && !(mapping)OCC_D->query_occ(norm)) {
        write("\"" + occ + "\" is not a valid OCC (not in the OCC list, "
            "and not \"none\"). OCC not changed.\n");
        return;
    }
    ob = find_player(pending_sub[player]);
    if(!ob) { write("Player not online.\n"); return; }
    ob->setenv("rifts_occ", norm);
