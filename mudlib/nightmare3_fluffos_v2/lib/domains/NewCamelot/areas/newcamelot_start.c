...
void reset() {
    ...
    if(!present("royal knight guard", this_object())) {
        k1 = clone_object("/domains/NewCamelot/npcs/royal_knight_guard");
        if(k1) k1->move(this_object());
        /* Two identical guards stand post here (see set_long()/set_items()
           above). Both independently hear and match the same "hello"/
           default response through rifts_npc.c's catch_tell(), so without
           this the same line prints twice, back to back. Blank the
           second guard's responses (the existing "no response yet" idiom
           from rifts_npc.c's own create()) so only one guard greets. */
        k2 = clone_object("/domains/NewCamelot/npcs/royal_knight_guard");
        if(k2) {
            k2->add_response("hello", "");
            k2->add_response("hi", "");
            k2->add_response("bye", "");
            k2->set_default_response("");
            k2->move(this_object());
        }
    }
}
