...
void catch_tell(string str) {
    ...
    if(sscanf(str, "%s says, \"%s\"", a, b) == 2) {
        b = lower_case(b);
        if(strsrch(b, "harbor") != -1 || strsrch(b, "scout") != -1) {
            tp = this_player();
            if(tp && objectp(tp)) call_out("offer_harbor_quest", 2, tp);
        }
    }
    ::catch_tell(str);
}

void offer_harbor_quest(object tp) {
    if(!tp || !objectp(tp)) return;
    if((int)QUEST_D->is_quest_complete(tp, "harbor_watch") ||
       (int)QUEST_D->is_quest_active(tp, "harbor_watch")) return;
    say_here("Elena says: 'Go down to the harbor and see what is coming in "
             "off the water. I cannot watch it every night myself. Report "
             "back what you find and I will make it worth the walk.'");
    catch(QUEST_D->start_quest(tp, "harbor_watch"));
}
