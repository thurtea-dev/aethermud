...
void init() {
    ...
    if(!(int)QUEST_D->is_quest_active(this_player(), "harbor_watch")) return;
    call_out("trigger_harbor_watch", 1, this_player());
}

void trigger_harbor_watch(object player) {
    if(!player || !objectp(player)) return;
    if(environment(player) != this_object()) return;
    if(!(int)QUEST_D->is_quest_active(player, "harbor_watch")) return;
    QUEST_D->update_objective(player, "harbor_watch", "harbor_scouted", 1);
    if(present("wild vampire", this_object()))
        tell_object(player,
            "Shapes move against the boats. Whatever Elena was worried "
            "about, it is here tonight.\n");
    else
        tell_object(player,
            "The harbor is quiet. Boats at anchor, nothing moving on the "
            "water. Worth reporting either way.\n");
    QUEST_D->complete_quest(player, "harbor_watch");
}
