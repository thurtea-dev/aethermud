// /domains/DemonPlane/areas/demon_sliver_shrine.c
...
void create() {
    ...
    set_exits( ([
        "south" : "/domains/DemonPlane/areas/demon_sliver_corridor"
    ]) );
    set_items( ([
...
void reset() {
    ...
    if(!present("demon sliver alpha", this_object()))
        new("/domains/DemonPlane/monsters/demon_sliver_alpha")->move(this_object());
    if(!present("rift shard amulet", this_object()))
        new("/domains/DemonPlane/equipment/rift_shard_amulet")->move(this_object());
}
