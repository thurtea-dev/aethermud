...
void catch_tell(string str) {
    ...
    if(strsrch(b, "dog") != -1 || strsrch(b, "shore") != -1) {
        tp = this_player();
        if(tp && objectp(tp)) call_out("offer_dog_quest", 1, tp);
        return;
    }
...
void offer_dog_quest(object tp) {
    if(!tp || !objectp(tp)) return;
    if((int)QUEST_D->is_quest_complete(tp, "lazlo_shore_dog") ||
       (int)QUEST_D->is_quest_active(tp, "lazlo_shore_dog")) return;
    say_here("Guard says: 'There is a feral dog working the lake shore, "
             "snapping at traders. Drive it off before someone gets bitten "
             "and I will see you paid for the trouble.'");
    catch(QUEST_D->start_quest(tp, "lazlo_shore_dog"));
}
