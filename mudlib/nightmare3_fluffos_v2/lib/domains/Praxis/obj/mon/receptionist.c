...
void create() {
    monster::create();
      ...
      set_long( "Linfield is the dignified receptionist for the "
	       "Praxis Inn. He stands tall behind the counter, "
...
int cmd_list(string str) {
  write( "%^GREEN%^ -= The Praxis Inn =-%^RESET%^");
  write( "\n");
...
int *arr;
    ...
    message("my_action", sprintf("You rent room %d of the Praxis Refuge Inn.",
      x), this_player());
    message("other_action", sprintf("%s rents a room at the Praxis Refuge Inn.",
      (string)this_player()->query_cap_name()), environment(this_object()),
      ({ this_player() }) );
    ob = new(OBJECT);
    ob->set_name("hotel key");
    ob->set_short("a hotel key");
    ob->set_long(sprintf("The key to room %d at the Praxis Refuge Inn.", x));
    ob->set_mass(50);
    ob->set_value(100);
    ob->set_read(sprintf("The Praxis Refuge Inn.\nRoom %d.\n", x));
