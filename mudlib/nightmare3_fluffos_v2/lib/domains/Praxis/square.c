...
void create() {
    ...
    set_long(
        ...
        "the town's people gather in joy and sorrow. The road running "
        "north and south is called Centre Path, while Boc La Road is the "
        "name of the road running east and west. A magnificent monument "
...
void reset() {
    ::reset();
    if(!present("monument", this_object()))
        new("/domains/Praxis/obj/misc/watchtower")->move(this_object());
    if(!present("bounty board", this_object()))
        new("/domains/Praxis/obj/bounty_board")->move(this_object());
}
