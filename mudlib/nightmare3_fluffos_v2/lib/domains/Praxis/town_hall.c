...
void create()
{
    ...
    set_short("Praxis Civic Hall");
    set_long(
        "A reinforced civic hall serving Praxis refugees. Concrete columns\n"
        "support a vaulted ceiling hung with faded pre-Rifts banners. A wide\n"
        "staircase leads up to the council chambers. The street is north.\n"
        "South lies the Court of Justice.");
    set_exits( ([
	"north" : "/domains/Praxis/e_boc_la1",
	"south" : "/domains/Praxis/court_room",
    ]) );
    add_exit( "up", "/domains/Praxis/council", (: "pre_exit_up" :) );
    set_items( ([
    ...
    set_listen( "council", (: "listen_council" :) );
}
