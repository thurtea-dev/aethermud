...
void reroute_sun_vulnerable_start(string race);
...
void do_plain_rolls() {
    ...
    int prev_iq, prev_me, prev_ma, prev_ps, prev_pp, prev_pe, prev_pb, prev_spd;
    int have_previous;
    int rolls;
    ...
    have_previous = (rolls > 0);
    if(have_previous) {
        prev_iq  = (int)player->query_stats("IQ");
        prev_me  = (int)player->query_stats("ME");
        prev_ma  = (int)player->query_stats("MA");
        prev_ps  = (int)player->query_stats("PS");
        prev_pp  = (int)player->query_stats("PP");
        prev_pe  = (int)player->query_stats("PE");
        prev_pb  = (int)player->query_stats("PB");
        prev_spd = (int)player->query_stats("Spd");
    }
    ...
    if(have_previous) {
        write(sprintf(
            " Previous: IQ:%-3d ME:%-3d MA:%-3d PS:%-3d\n"
            "           PP:%-3d PE:%-3d PB:%-3d Spd:%-3d",
            prev_iq, prev_me, prev_ma, prev_ps,
            prev_pp, prev_pe, prev_pb, prev_spd));
        write(sprintf(
            " New:      IQ:%-3d ME:%-3d MA:%-3d PS:%-3d\n"
            "           PP:%-3d PE:%-3d PB:%-3d Spd:%-3d",
...
private int do_race_pick(string str) {
    ...
    reroute_sun_vulnerable_start(str);
...
/* Sun-vulnerable races must not finish chargen pointed at a daylight-
   exposed start room: daemon/rifts_combat.c do_sunlight_exposure() would
   start taking 2d6 MDC off them on their first gated heartbeat, before
   they have gear or any idea what is happening.

   Zone is chosen at step 1 and race later, so the start room is already
   set by set_primary_start() before we know the race -- hence the override
   here rather than at region selection.

   Of the three zone starts only Atlantis is exposed: chitown_start and
   newcamelot_start both set indoors 1, splynn_entry sets indoors 0.
   Rather than add a room, this reuses the existing indoor
   rockys_bar_splynn ("Rocky's Bar, Splynn"), which is in the same zone
   graph and reads naturally as arriving under cover. Driven off the
   race's own sunlight_vulnerability flag, not a hardcoded vampire list,
   so any future sun-vulnerable RCC is covered automatically. */
void reroute_sun_vulnerable_start(string race) {
    string *flags;
    string region;
    string safe_room;

    if(!race || !strlen(race)) return;
    flags = (string *)RIFTS_D->query_race_flags(race);
    if(!flags || member_array("sunlight_vulnerability", flags) == -1) return;

    region = (string)this_player()->getenv("home_region");
    if(region != "atlantis") return;

    safe_room = "/domains/Praxis/areas/splynn/rockys_bar_splynn";
    if(file_size(safe_room + ".c") <= 0) return;

    this_player()->set_primary_start(safe_room);
    write("\nSplynn's arrival plaza stands open to the sun, which would kill "
          "you\nwhere you stood. You will begin instead under cover, inside "
          "Rocky's Bar.");
}
