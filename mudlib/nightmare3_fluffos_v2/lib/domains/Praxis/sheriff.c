...
void create() {
    ...
    set_properties( ([ "light":2, "indoors":1, "no bump":1, "no kill":1,
      "no steal":1 ]) );
    set_short("Praxis Security Office");
    set_long(
        "A reinforced security office for Praxis settlement law enforcement.\n"
        "A barred holding area occupies one corner. A scarred desk and wanted\n"
        "posters cover the walls. Exits to the court yard and court room\n"
        "are west and south.");
...
void control_townsfolk(mixed *them) {
  ...
  if(them[0]->query_ghost()) {
    /* "cheer" was a canned soul.c emote verb, retired 2026-07-26;
       emote is now the only action command, so route the crowd's
       reaction through it directly instead. */
    them[1]->command("emote cheers enthusiastically.");
